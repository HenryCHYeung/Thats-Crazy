...
def recommend_components(component_data, purpose, user_budget):
    purpose_map = {1: "Gaming", 2: "Production"}

    if purpose not in purpose_map:
        raise ValueError("Invalid choice. Please choose 1 for Gaming or 2 for Production.")

    purpose_name = purpose_map[purpose]
    components = component_data[purpose_name]

    # Determine the budget tier based on the user's budget
    budget_tier = "LowEnd" if (purpose == 1 and user_budget <= 1000) or (purpose == 2 and user_budget <= 1500) else "HighEnd"
    ...
    for component_type in ["CPUs", "GPUs", "RAM", "Storage", "PSU", "Case", "Motherboard"]:
        affordable_components = [
            component
            for component in components[budget_tier][component_type]
...
def main():
    ...
    while True:
        ...
        try:
            recommendations = recommend_components(component_data, user_usage, user_budget)
